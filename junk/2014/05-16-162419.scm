  ...
  (let ((type-env (lookup type env)))
    (if (pair? type-env)
        (let ((op-env (assq op (cdr type-env))))
          (if (pair? op-env)
              (cdr op-env)
	      #f));(error "undefind operand" op)))
	#f)));(error "undefind type" type))))
...
;;sicp2.78
;attach-tagの変更最初いらないおおもったけど、
;scheme-numberじゃなくて数字のみにするために必要
(define (attach-tag type-tag contents) 
...
(define a (make-complex-from-real-imag 2 3))
(define b (make-complex-from-real-imag 5 4))
;(print (exp 1 2))
;(exp a b)
